...
# 【V4 構成】 GPU少数精鋭 & データ供給効率化
# 8GB VRAM内に収めるため、Actorを2人に絞り、その分データ生成効率(Batch送付)でカバーします
NUM_ACTORS = 2 
ACTOR_GPU_FRACTION = 0.1   # GPU使用 (Mamba必須)
LEARNER_GPU_FRACTION = 0.6 # GPU使用

ACTOR_BATCH, SEQ_LEN = 4, 32
# バッチで一気に入れるので、Replay容量とバックプレッシャーの閾値を少し調整
...
class MambaModel(nn.Module):
    def __init__(self, vocab_size, d_model, d_state, d_conv, expand):
        ...
        try:
            ...
            # ここで落ちるとRayの初期化エラーとして捕捉されます
            raise ImportError("❌ mamba_ssm is required! Make sure it's installed and CUDA is available.")
    ...
    def train_loop(self, max_steps):
        try:
            ...
            while step < max_steps:
                # Replayが貯まるまで待機 (ここがボトルネックにならないか監視)
...
class SelfPlayActor:
    def __init__(self, actor_id, replay_actor, param_server):
        try:
            ...
            self.device = torch.device(DEVICE) # GPU使用
            ...
            print(f"❌ Actor {actor_id} Init Error: {e}")
            traceback.print_exc()
            raise e
    ...
    def step(self):
        try:
            ...
            with torch.inference_mode():
                ...
                # CPUへ送ってからリスト化 (VRAM節約)
                samples = [(inp.cpu()[i], tgt.cpu()[i]) for i in range(ACTOR_BATCH)]
            
            # 【V4改善】 4つまとめて送る (通信効率向上)
            self.replay.add_batch.remote(samples)
            return "gen"

        except Exception as e:
            # 明示的に raise して Driver に RayActorError を投げさせる
            print(f"❌ Actor {self.actor_id} Step Error: {e}")
            traceback.print_exc()
            raise e
...
def start():
    ...
    print(f"🚀 Project initialized. Actors({NUM_ACTORS}) & Learner on GPU...")
    ...
    try:
        while len(pending_tasks) > 0:
            ...
            for done_id in done_ids:
                ...
                try:
                    _ = ray.get(done_id) # 結果取得 (ここで raise された例外をキャッチ)
                    ...
                    print(f"\n⚠️ Actor task failed. Removing actor from pool. Error: {e}")
                    # 再スケジュールしない = このActorは脱落
                    continue
