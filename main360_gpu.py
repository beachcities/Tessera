...
# 【修正1】ActorをCPUへ完全退避 (VRAM 8GB対策の要)
# Learnerに全てのVRAM (8GB) を捧げます
NUM_ACTORS = 8 
ACTOR_GPU_FRACTION = 0
LEARNER_GPU_FRACTION = 0.9 # RTX 4070 Laptopは1枚なので実質1.0に近い割り当て
...
class Learner:
    ...
    def train_loop(self, max_steps):
        # 【修正2】例外ハンドリングの追加 (Silent Crash防止)
        try:
            step = 0
            loss_history = []
            ...
            traceback.print_exc() # これでログにスタックトレースが出る
            raise e
