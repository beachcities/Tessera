...
def start_training():
    ...
    for step in range(1, 10001):
        ...
        # タイムアウトを少し長めに (初回コンパイル待ち対策)
        ready, remaining = ray.wait(futures, num_returns=len(futures), timeout=30)

        if len(ready) == 0:
            if step == 1: print("⏳ DEBUG: Waiting for actors (Compilation)...")
            time.sleep(0.1)
            continue
