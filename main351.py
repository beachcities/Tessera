...
# ==========================================
# 【Block 1】 環境設定 (Local RTX 4070 Optimized)
# ==========================================
...
# ==========================================
# 【Block 2】 モデル定義 (GoMambaSpatialNet)
# ==========================================
...
# ==========================================
# 【Block 4】 分散Actor管理 (Local Optimized)
# ==========================================
# 既存のRayプロセスがあれば掃除
...
class SelfPlayActorV3:
    def __init__(self, sd_ref):
        # Actor初期化時のログ
        print("🤖 Actor: Initializing...")
        sd_dict = ray.get(sd_ref)
        self.device = 'cpu'
        self.dtype = torch.float32
        
        self.model = GoMambaSpatialNet().to(self.device).to(self.dtype)
        cpu_sd = {k: v.to(self.device).to(self.dtype) for k, v in sd_dict.items()}
        self.model.load_state_dict(cpu_sd)
        self.env = TorchGo(batch_size=64, device=self.device)
        print("🤖 Actor: Ready.")

    def play_and_report(self, sd_ref=None):
...
def start_training():
    ...
    print(f"👥 DEBUG: Spawning {num_actors} CPU Actors via Ray...")
...
# ==========================================
# 【Main Execution Block】 (Error Handling)
# ==========================================
if __name__ == "__main__":
    try:
        # 出力バッファを強制フラッシュ（ログを即時表示）
